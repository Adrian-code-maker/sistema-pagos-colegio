...
app.post('/api/registrar', async (req, res) => {
    ...
    try {
        ...
        const resUsuario = await client.query(
            `INSERT INTO usuarios (nombre_completo, cedula, correo, contrasena, rol, estatus) VALUES ($1, $2, $3, $4, 'representante', 'pendiente') RETURNING id;`,
            [nombre_completo, cedula, correo, contrasenaEncriptada]
        );
        ...
        if (alumnos && alumnos.length > 0) {
            for (let alumno of alumnos) {
                ...
                if (isGrado && (!alumno.seccion || alumno.seccion === 'Ninguna')) throw new Error(`Falta la sección para el alumno ${alumno.nombre_completo}`);
                if (!isGrado && (!alumno.mencion || alumno.mencion === 'Ninguna')) throw new Error(`Falta la mención para el alumno ${alumno.nombre_completo}`);
        ...
        res.status(201).json({ ok: true, mensaje: 'Usuario registrado con éxito.' });
    } catch (error) {
